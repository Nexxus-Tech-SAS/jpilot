...
**Current release:** `v0.121` (beta) — **Deck layout hardening for live presenting.** The `/deck` presentation now renders on a fixed 1440×810 (16:9) canvas that scales as a whole to fit any screen — laptop, projector, 4K, or a small window — so every slide is pixel-identical regardless of viewport (fixing titles/panels that could clip on scaled displays). Viewport-relative type (`clamp`/`vw`) was replaced with fixed sizes that scale with the canvas. The four two-column slides (vision, architecture, Calibration Studio, why-now) had their stacked left-column cards touching with no gap; they now use a flex stack with consistent 16px spacing, and the column gap widened to 32px. The mobile single-column breakpoint was removed (it fought the fixed canvas); background lattice, auroras, and chrome stay viewport-sized so the frame always fills the screen.
