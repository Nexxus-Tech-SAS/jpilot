...
**Current release:** `v0.93` — **NetScaler goal tools verified end-to-end in JPilot Chat.** Closed two wiring gaps found by the chat e2e: the backend `mcpSettings` tool registry (`mcp_config_service.ALL_TOOL_NAMES`) now lists all 15 new tools so they actually reach the model, and the `config_search` intent router matches broader "running config" phrasing. Verified: chat (operator role) selects `netscaler_get_logs`, `netscaler_create_lb` (dry-run), and `netscaler_search_config` for natural-language requests, with the search tools demoted.
