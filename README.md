...
**Current release:** `v0.90` — **NetScaler tooling baseline.** Added the NetScaler MCP Tools capability plan ([`docs/netscaler-tools-plan.md`](docs/netscaler-tools-plan.md)) mapping the desired Load-Balancing / Content-Switching / Rewrite / Responder lifecycles to dedicated goal-based MCP tools (declarative `applications`/`config_sets` first, guarded CLI fallback), and fixed `netscaler_add_ip_address` to tolerate NITRO's empty `201 Created` body. Baseline checkpoint before the goal-tool build-out.
