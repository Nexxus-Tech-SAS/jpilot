...
**Current release:** `v0.74` — **One-click update button in consolidated Settings.** Fixed the About & updates panel not showing the admin "Update to vX.Y" button: after settings were consolidated onto `SettingsBetaView`, the `about` section rendered `UpdatesPanel` without passing `isAdmin`, so the button (gated on `isAdmin && update_available`) stayed hidden behind the manual instructions. The section now passes `isAdmin`.
