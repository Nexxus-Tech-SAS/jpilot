...
**Current release:** `v0.87` — **Loop-breakers: JPilot stops and asks instead of grinding.** When a JPilot chat turn gets stuck — the same tool call failing over and over, one tool failing repeatedly, or a run of steps that produces nothing new — JPilot now **pauses and hands control back to you** with a short summary of what it tried, the last error, and a request for the missing detail or a different approach, instead of silently burning through its tool-call budget. Detection is conservative (it trips only on clear signals, well past a floor of tool rounds) and the thresholds are **admin-configurable** under Settings → JPilot orchestration. Covers both streaming and standard chat.
