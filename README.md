...
**Current release:** `v0.122` (beta) — **Switchable UI V2 preview — a full interface redesign you can opt into.** Settings gains an **Appearance** section (visible to every user) with a **"New JPilot interface"** toggle that swaps the entire app live — no reload — between the classic UI and a redesigned V2 experience; a **"New UI · Beta"** pill inside V2 switches back instantly. V2 brings an animated glass navigation rail (sliding gradient active indicator, staggered entrances, collapsible icon rail, morphing sun/moon theme toggle), a restructured chat (centered readable message column, gradient user bubbles, animated message entry, shimmer loading state, pill composer with glow focus, compact tool-trace cards, modernized conversation sidebar), reskinned dashboard / settings / Calibration Studio / plans / login views, a PrimeVue-wide retheme (glass dialogs, drawers, and toasts, a new overlay motion language, themed scrollbars), and first-class phone/iPad support (floating bottom dock, slide-in drawer, tablet icon rail, safe-area insets, 44px touch targets, full `prefers-reduced-motion` support) — including a fix for the classic iPad-portrait dead zone where chat conversations could not be switched. The implementation is fully additive: a `ui-v2` class on `<html>` (mirroring the dark-mode mechanism) gates six new stylesheets and a parallel `MainLayoutV2` shell, the classic UI renders unchanged when the toggle is off and remains the default, and the choice persists per the cookie-consent rules. Architecture and adoption notes in `docs/ui-v2.md`.

**Previous release:** `v0.121` (beta) — **Deck layout hardening for live presenting.** The `/deck` presentation now renders on a fixed 1440×810 (16:9) canvas that scales as a whole to fit any screen — laptop, projector, 4K, or a small window — so every slide is pixel-identical regardless of viewport (fixing titles/panels that could clip on scaled displays). Viewport-relative type (`clamp`/`vw`) was replaced with fixed sizes that scale with the canvas. The four two-column slides (vision, architecture, Calibration Studio, why-now) had their stacked left-column cards touching with no gap; they now use a flex stack with consistent 16px spacing, and the column gap widened to 32px. The mobile single-column breakpoint was removed (it fought the fixed canvas); background lattice, auroras, and chrome stay viewport-sized so the frame always fills the screen.

**Previous release:** `v0.120` (beta) — **JPilot presentation deck at `/deck`.** A built-in, 16-slide product deck (problem, vision, product, roles, architecture, agent loop, security, Calibration Studio, live-demo flow, roadmap, pricing, team, ask) now ships in the app as a public route — same access rules as the landing page. It uses the new brand system end to end: the animated `JPilotMark` orb on the title/close slides and chrome, gradient JPilot wordmarks, and the cyan→indigo ramp across the progress bar, hero shine, network-lattice background, and aurora backdrops. Presentation polish: direction-aware slide transitions (enter/exit follow navigation direction with blur-scale settle), auto-staggered per-element reveals with type-specific motion (3D card tilt-in, clip-path title wipe, eyebrow letter-spacing settle), keyboard navigation (arrows / Space / PageUp·Down / Home / End) with focus capture on load, touch swipe, clickable dots, a fullscreen toggle (button + `F` key), and full `prefers-reduced-motion` support. The route is lazy-loaded so the main bundle is unaffected.

**Previous release:** `v0.119` (beta) — **Brand identity refresh + copilot reliability.** JPilot has a new logo system: an animated **orb nexus** full mark (a glowing AI core with a live waveform, wired to three appliance nodes with flowing command packets) on login, account recovery, and the landing hero, and a matching **pulse mark** compact icon (same orb ring, waveform, and satellite nodes) in the nav, chat, legal header, favicons, and avatars — both built from one `JPilotMark.vue` component with a shared cyan→indigo ramp, theme-aware colors (deeper ramp + dark core disc for light mode legibility), and `prefers-reduced-motion` support. The **landing page** was re-skinned to the new palette (navy-cyan hero, gradient buttons/eyebrows, recolored constellation) with motion borrowed from the logo (breathing glows, hover ring-glows); the **login page** aurora and accents now follow the same ramp. Brand prominence was raised everywhere: a JetBrains Mono gradient **JPilot wordmark** (with the classic blinking cursor) now sits beside/under the mark on the nav rail, login (2.25rem), chat sidebar, legal header, and landing hero lockup. Favicons, apple-touch icons, and `favicon.ico` were regenerated from the new mark. Also included (previously unreleased): copilot tool-call reliability fixes (parallel tool_use/tool_result 400s, live Anthropic model list), blueprint pinning across multi-turn flows with an intent-gated matcher, out-of-band secret masking for chat credentials, LB provisioning via service groups with named-server binds, Operator/Architect prompt+budget tuning, and NetScaler rewrite quoting fixes for security-header blueprints.

**Previous release:** `v0.118` (beta) — **Calibration Studio blueprint card polish.** Blueprint library cards use subtle tier tints (Free vs Ent / Ent+) on a glass panel base, plus the same accent hover glow as the home role tiles — lift, colored border, and soft shadow keyed to tier (primary, blue, violet).
...
**Earlier release:** `v0.103` (beta) — **O6 part 3: single LB executor.** The legacy natural-language / form LB auto-deploy now routes its WRITE through `netscaler_create_lb` instead of hand-built raw CLI — so there is one executor for LB provisioning. The battle-tested cleanup/verify and idempotent re-deploy are preserved (re-deploy now removes the old vserver via `netscaler_delete_lb` first), multi-backend is supported, and the plan → approve → confirm handshake is unchanged. Behavior change (intended): auto-deployed LBs now use the individual-service model (with optional health monitor) instead of `serviceGroup`. NL parsing was also widened (multi-backend, port/method/persistence detection). Live-verified end-to-end on the lab appliance; `test_copilot_deploy.py` 15/15.

**Previous release:** `v0.102` (beta) — **O6 part 2: `create_lb` health-monitor superset.** Extended `netscaler_create_lb` with an optional `monitor` parameter (binds the named health monitor to each backend service), threaded through the MCP tool and the Copilot schema/dispatch — bringing the dedicated tool to parity with the legacy auto-deploy builder ahead of the executor flip. Optional and backward-compatible (existing calls unchanged); dry-run/confirm preserved; 3 new unit tests. Note: NetScaler refuses an explicit bind of the always-present `tcp-default` monitor, so the upcoming executor flip will skip default monitors.

**Previous release:** `v0.101` (beta) — **O6 part 1: close the raw-CLI LB escape.** Caught by the live log watcher: the operator chat model was hand-building raw `add lb vserver … -purpose lb …` (an invalid arg) via `run_cli_command` instead of using `netscaler_create_lb`. Two layers now prevent that: (1) a backstop that blocks any raw `run_cli_command(s)` containing an LB-provisioning verb (`add lb vserver` / `add serviceGroup` / `bind lb vserver`) and redirects the model to `netscaler_create_lb` (reads like `show lb vserver` are unaffected); (2) the router no longer re-exposes raw `run_cli_command(s)` on an LB-intent or LB-confirm turn (the affirmation/"yes" turn keeps the narrow dedicated packs instead of falling back to the full tool set). Executor unification (routing the legacy auto-deploy write through `create_lb`) follows in a later increment. Design: [`docs/role-audits/o6-plan.md`](docs/role-audits/o6-plan.md).

**Previous release:** `v0.100` (beta) — **SSH-read gate tidy-up.** Read-only `*_ssh_run_command` `show`/`stat`/`get` commands no longer force a `search_netscaler_cli_reference` round-trip (the CLI-memory gate is now arg-aware for the SSH tools, mirroring the `run_cli_command` exemption), so operator/analyst reads are faster — the reported "show all virtual servers" query now resolves in 2 read calls with no forced search. Caps the `netscaler-goal-tools` beta line so far: 15 dedicated NetScaler goal tools (LB / CS / Rewrite / Responder + logs / config-search / failover), role-workflow optimizations (analyst / operator / architect), and the skippable blueprint-suggestion button.

**Previous release:** `v0.99` — **Read-query false-positive fixes (from chat feedback).** "Deployment may be incomplete" banner and the LB-create form no longer fire on read/show queries. Two operator-role bugs on a pure read ("show me all virtual servers configured and the backend services"): the "Deployment may be incomplete" banner now only fires after an actual LB write this turn (a read/listing of existing DOWN vservers no longer trips it), and the LB-create form no longer auto-attaches to read/show requests (the create-intent detector was matching "configured" as the verb "configure"; now short-circuits on read markers without an explicit create imperative). Live-verified.

**Previous release:** `v0.98` — **Blueprint button: always shown + skippable.** The blueprint card now appears whenever a blueprint is relevant, not only on a match miss. Two states: `applied` ("Using the X blueprint for this answer" with a **Use standard model instead** action) and `suggested` ("This looks like the X blueprint" with **Apply** / **Skip**). **Skip** re-runs the original message with the blueprint suppressed (new `ChatRequest.skipBlueprintSkillId`), so the user can always see which blueprint is in play and opt out to the standard model. Live-verified on deepseek-v4-pro.

**Previous release:** `v0.97` — **Blueprint suggestion button.** When the automatic blueprint/calibration match misses, JPilot now surfaces the likely blueprint as a clickable card (**Apply blueprint** / **Skip — use standard model**) instead of silently falling back to the generic model. Backend: optional `blueprintSuggestion` on `ChatResponse`, populated by a relaxed candidate match behind a confidence floor (one suggestion per turn) — gated to architect at conversation start and to operator/analyst on a dedicated config/read intent, and only when blueprints are installed for the role/vendor. Frontend: a `ChatBlueprintSuggestion` card reusing the existing actionable-widget pattern; Apply sends the blueprint's apply-prompt, Skip dismisses client-side.

**Previous release:** `v0.96` — **Role-workflow optimizations (do-now tier).** From the workflow review, shipped the high-value low-risk fixes per role. **Operator:** the confirmation turn now routes straight to the dedicated `create_*`(confirm=true) tool (no more raw-CLI fallback), the destructive-confirmation gate understands the dedicated tools, a successful `create_*` counts as a completed write (no spurious "continue?" pause), LB intake is one consolidated form, and dedicated-config form-submit turns no longer re-open the gated raw-CLI path. **Analyst:** read-only `nextgen_get` and `show/stat` CLI no longer force a "search reference first" round-trip, and `get_logs`/`search_config` are now first-class analyst tools (still strictly read-only). **Architect:** discovery groups related fields into fewer forms, readiness is topic-coverage-based (not click-count) with a hard backstop so users can't be trapped in endless discovery, and deliverables follow a structured section skeleton. Write confirmation preserved throughout.

**Previous release:** `v0.95` — **Role-workflow fixes (analyst / operator / architect).** Audited each chat role (results in [`docs/role-audits/`](docs/role-audits/)) and fixed: **analyst** no longer shows false "unverified read / no changes applied" banners on the SSH-backed read tools (`get_logs`, `search_config`); **operator** config intents no longer offer the gated raw `run_cli_command` (avoiding the blocked-CLI "stuck" loop), gate-blocks/confirmation-waits no longer count toward the stuck threshold, and the operator prompt + raw-CLI tool descriptions steer to the dedicated `create_*` tools; **architect** `user_wants_deliverable_now()` recognizes broader "give me the design" phrasing with an escape-hatch hint. Write confirmation (dry-run → approve → confirm) preserved.

**Previous release:** `v0.94` — **Tighten config-intent routing + steer prompt to dedicated tools** (precise-intent routing so provisioning requests don't fall to the full 45-tool set; system prompt prefers dedicated tools).

**Previous release:** `v0.93` — **NetScaler goal tools verified end-to-end in JPilot Chat.** Closed two wiring gaps found by the chat e2e: the backend `mcpSettings` tool registry now lists all 15 new tools so they reach the model, and the `config_search` router matches broader "running config" phrasing.

**Previous release:** `v0.92` — **NetScaler goal tools wired into Copilot.** The 15 new NetScaler tools are now exposed to JPilot Chat: copilot catalog entries (`appliance_name`-based, credentials resolved server-side), `MCP_TOOL_MAP` + dispatch arg-forwarding, write-execution gating (`WRITE_EXEC_TOOL_NAMES`), and the admin tool catalog. Intent routing maps "create a load balancer / content switch / rewrite / responder", log, config-search, and failover requests to the dedicated tools, and **demotes the CLI/Next-Gen search tools to last resort** (routing + tool descriptions + a system-prompt preference line). Analyst stays read-only; write tools require operator. Router-unit verified.
...
**Previous release:** `v0.89` — **Motion-branded login and chat chrome.** Login and JPilot Chat now share a cohesive animated identity: a **Tri-Arc** spinner replaces static logos (login, chat sidebar, main menu, and mobile chat header), **Term-OS** blinking-cursor titles mark key labels (`JPilot`, role names), and login actions use **Glass + Magnetic** button effects. Role labels in chat use theme text colour on desktop and role-accent chips on mobile. All motion is pure CSS with `prefers-reduced-motion` support.

**v0.88** — **Tune or disable stuck detection from Settings.** The loop-breakers introduced in v0.87 are now exposed in **Settings → JPilot → Agent orchestration**: a master **on/off toggle** plus the four thresholds — identical failed calls before pausing, failures of one tool before pausing, no-progress steps before pausing, and the minimum tool rounds before the no-progress check. They apply across every orchestration mode (Standard / Extended / Max / Custom) and persist in the platform settings.

**v0.87** — **Loop-breakers: JPilot stops and asks instead of grinding.** When a JPilot chat turn gets stuck — the same tool call failing over and over, one tool failing repeatedly, or a run of steps that produces nothing new — JPilot **pauses and hands control back to you** with a short summary of what it tried, the last error, and a request for the missing detail or a different approach, instead of silently burning through its tool-call budget. Detection is conservative (it trips only on clear signals, well past a floor of tool rounds) and covers both streaming and standard chat. *(Settings → JPilot controls for these thresholds were added in v0.88.)*
...
| Area | Highlights |
|------|------------|
| **JPilot Chat** | Fixed PrimeVue `SelectButton` role picker prop warnings (`option-label="label"`). Added `id`/`name` on composer, attach, and calibration form fields. |
| **Dashboard** | Nexxus blog articles fetched through backend `GET /system/nexxus-blog` (server-side proxy) — eliminates browser CORS failures from `/nexxus-blog` redirects. Nginx dev proxy target updated to `nexxus-tech.com` (no www). |
