...
**Current release:** `v0.97` — **Blueprint suggestion button.** When the automatic blueprint/calibration match misses, JPilot now surfaces the likely blueprint as a clickable card (**Apply blueprint** / **Skip — use standard model**) instead of silently falling back to the generic model. Backend: optional `blueprintSuggestion` on `ChatResponse`, populated by a relaxed candidate match behind a confidence floor (one suggestion per turn) — gated to architect at conversation start and to operator/analyst on a dedicated config/read intent, and only when blueprints are installed for the role/vendor. Frontend: a `ChatBlueprintSuggestion` card reusing the existing actionable-widget pattern; Apply sends the blueprint's apply-prompt, Skip dismisses client-side.
