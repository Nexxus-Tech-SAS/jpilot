...
**Current release:** `v0.84` — **Faster Docker image builds.** Backend API, MCP server, and frontend images use BuildKit cache mounts for `pip`/`npm`, `npm ci` with lockfiles, and the JPilot frontend dev container no longer runs `npm install` on every start.
