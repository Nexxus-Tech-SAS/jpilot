...
const props = defineProps({
  linesGradient: { type: Array, default: () => [] },
  ...
  topWavePosition: { type: Object, default: undefined },
  middleWavePosition: { type: Object, default: undefined },
  bottomWavePosition: { type: Object, default: () => ({ x: 2.0, y: -0.7, rotate: -1 }) },
...
function mountFloatingLines() {
  const container = containerRef.value
  if (!container) return null

  const enabledWaves = props.enabledWaves
  ...
  function renderLoop() {
    ...
    uniforms.iTime.value = animationDisabled ? 0 : clock.getElapsedTime()
    ...
    if (props.parallax && props.interactive) {
...
watch(
  () => [
    props.linesGradient?.join(','),
    props.enabledWaves.join(','),
    JSON.stringify(props.lineCount),
    JSON.stringify(props.lineDistance),
    JSON.stringify(props.topWavePosition),
    JSON.stringify(props.middleWavePosition),
    JSON.stringify(props.bottomWavePosition),
