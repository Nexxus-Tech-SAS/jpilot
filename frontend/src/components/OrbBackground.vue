<template>
  <div ref="containerRef" class="orb-container" aria-hidden="true" />
</template>
...
function mountOrb() {
  ...
  function update(t) {
    ...
    const dt = animationDisabled ? 0 : (t - lastTime) * 0.001
    lastTime = t

    program.uniforms.iTime.value = animationDisabled ? 0 : t * 0.001
    ...
    const effectiveHover =
      props.enableMouseInteraction && (props.forceHoverState ? 1 : targetHover)
    program.uniforms.hover.value += (effectiveHover - program.uniforms.hover.value) * 0.1

    if (props.rotateOnHover && effectiveHover > 0.5 && !animationDisabled) {
...
watch(
  () => [props.hue, props.hoverIntensity, props.backgroundColor, props.rotateOnHover, props.forceHoverState],
  () => {
