<template>
  <div ref="containerRef" class="galaxy-container" aria-hidden="true" />
</template>
...
function applyUniforms(program) {
  program.uniforms.uFocal.value[0] = props.focal[0]
  program.uniforms.uFocal.value[1] = props.focal[1]
  program.uniforms.uRotation.value[0] = props.rotation[0]
  program.uniforms.uRotation.value[1] = props.rotation[1]
  program.uniforms.uStarSpeed.value = props.starSpeed
...
function mountGalaxy() {
  ...
  animationDisabled =
    props.disableAnimation || window.matchMedia('(prefers-reduced-motion: reduce)').matches
