<template>
  <div ref="containerRef" class="threads-container" aria-hidden="true" />
</template>
...
function mountThreads() {
  ...
  function update(t) {
    ...
    if (animationDisabled || !isVisible || document.hidden) return
...
watch(
  () => [props.color[0], props.color[1], props.color[2], props.amplitude, props.distance],
  () => {
    if (!programRef) return
    programRef.uniforms.uColor.value.set(...props.color)
    programRef.uniforms.uAmplitude.value = props.amplitude
    programRef.uniforms.uDistance.value = props.distance
  }
)
